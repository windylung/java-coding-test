...
### 문제 풀이 후기
문제 내 조건이었던 "10%를 계산한 금액이 1원 미만인 경우에는 이득을 분배하지 않고 자신이 모두 가집니다"을
기존은 if 문을 사용해 처리하였다.
...
```
다음과 같이 90%를 올림처리, 10%를 내림처리 하게 된다면
1원 미만의 값들은 자동으로 분배되지 않는다.
알고리즘을 풀면 코드가 더럽다는 느낌을 받을 때가 있는데, 이번 문제처럼 불필요한 조건문이 많은 것도 원인인 듯 하다. 수학적으로 처리 방법도 많이 연습해보아야겠다.


### 성능 요약

메모리: 119 MB, 시간: 8947.45 ms
